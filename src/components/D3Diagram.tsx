...
class D3Diagram extends React.Component<D3DiagramProps, {}> {
  ...
  componentDidMount() {
    const { data, deviceColorMap } = this.props;

    const devicesWithAtLeastOneMeasurement = data.filter(
      ([_, measurements]) => measurements.length !== 0
    );

    if (devicesWithAtLeastOneMeasurement.length !== 0) {
      drawDiagram(this.d3Ref, data, deviceColorMap);
    }
  }

  render() {
    return <div style={{ width: "100%", height: "100%" }} ref={this.d3Ref} />;
  }
}
